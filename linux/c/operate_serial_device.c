...
int receiveTTyData(char *code_buf, int buf_len)
{
	...
	if(scanner_fd < 0) {
		printf("%s:%d  open_serial(%s) failed:%s", __func__, __LINE__, SCANNER_DEV_NOTE, strerror(errno));
		return "null";
	}
	printf("%s:%d The scannser file id:%d", __func__, __LINE__, scanner_fd);
	int ret_val = setup_port(scanner_fd, 9600, 8, 'N', 1);
	if(ret_val < 0) {
		printf("setup_port() failed:%d", ret_val);
		goto CLOSE_FILE_TAG;
	}
	ioctl(scanner_fd, 121, 1);
	ioctl(scanner_fd, 121, 0);

	memset(code_buf, 0, buf_len);
	int nread = 0, sum_read = 0;
	while((nread=read(scanner_fd, code_buf+sum_read, MAX_CODE_LENGHT-sum_read)) > 0 && sum_read < MAX_CODE_LENGHT ) {
		sum_read += nread;
		printf("GET CODE: nread=%d  %s", nread, code_buf);
	}
...
int open_serial(const char *dev_note)
{
	 ...
	 */
	printf("%s:%d open file:%s", __func__, __LINE__, dev_note);
	...
	if(file_d == -1) {
	    printf("%s:%d open(%s) failed:%s", __func__, __LINE__, dev_note, strerror(errno));
	    return -1;
	}
